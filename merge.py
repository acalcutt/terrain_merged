...
class ProcessTileArgs:
    ...
    resampling: int
    output_image_format: ImageFormat  # Added output image format
...
class TerrainRGBMerger:
    def __init__(
        ...
        output_image_format: ImageFormat = ImageFormat.PNG,  # Added output image format
    ):
        ...
        self.output_image_format = output_image_format  # Store the image format
    ...
    def _decode_tile(
        self, tile_data: bytes, tile: mercantile.Tile, encoding: EncodingType
        ...
        try:
            ...
            with rasterio.open(image_png) as dataset:
                ...
                if rgb.ndim != 3 or rgb.shape[0] != 3:
                    self.logger.error(
                        f"Unexpected RGB shape in tile {tile.z}/{tile.x}/{tile.y}: {rgb.shape}"
                    )
                    return None, {}
    ...
    def _extract_tile(
        self, source: MBTilesSource, zoom: int, x: int, y: int
    ) -> Optional[TileData]:
            ...
            with self._db_connection(source.path) as conn:
                ...
                if result is not None:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to decode tile {current_zoom}/{current_x}/{current_y}: {e}"
                        )
                        return None
    ...
    def _merge_tiles(
        self,
        primary: Optional[TileData],
        secondary: Optional[TileData],
        ...
        target_transform = rasterio.transform.from_bounds(
            ...
            bounds.north,
            tile_size,
            tile_size,  # Use determined tile size
        )
        ...
        if primary is not None:
            result = self._resample_if_needed(
                primary, target_tile, target_transform, tile_size
            )

        # Process and merge secondary tile if available
        if secondary is not None:
            secondary_data = self._resample_if_needed(
                secondary, target_tile, target_transform, tile_size
            )
            if result is None:
    ...
    def process_tile(self, tile: mercantile.Tile) -> None:
        ...
        try:
            ...
            primary_data = self._extract_tile(
                self.primary_source, tile.z, tile.x, tile.y
            )
            secondary_data = self._extract_tile(
                self.secondary_source, tile.z, tile.x, tile.y
            )
            ...
            if merged_elevation is not None:
                ...
                self.logger.info(
                    f"Successfully processed tile {tile.z}/{tile.x}/{tile.y}"
                )
        except Exception as e:
            ...
            if rgb_data.size > 0:
                if rgb_data.ndim == 3:
                    ...
                    image = Image.fromarray(
                        np.moveaxis(
                            np.zeros((3, tile_size, tile_size), dtype=np.uint8), 0, -1
                        ),
                        "RGB",
                    )
    ...
    def _get_tiles_for_zoom(self, zoom: int) -> List[mercantile.Tile]:
        ...
        with self._db_connection(source.path) as conn:
            ...
            if not rows:
                self.logger.warning(
                    f"No tiles found for zoom level {zoom} in source {source.path}"
                )
            else:
    ...
    def _process_tile_wrapper(args: ProcessTileArgs) -> None:
        ...
        try:
            merger = TerrainRGBMerger(
                ...
                output_image_format=args.output_image_format,  # Pass the image format
            )
...
if __name__ == "__main__":
    ...
    base_output_path = Path(f"/opt/output_webp_{timestamp}.mbtiles")
    ...
    merger = TerrainRGBMerger(
        ...
        output_image_format=ImageFormat.WEBP,  # Set the desired output format here
    )
